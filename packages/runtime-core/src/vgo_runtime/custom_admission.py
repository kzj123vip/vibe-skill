...
def _rule_task_allow(skill_id: str, routing_rules: dict[str, Any] | None) -> list[str]:
    if not isinstance(routing_rules, dict):
        return []
    rules = routing_rules.get("skills") or {}
    ...
    rule = normalized_rules.get(_normalize_text(skill_id)) or {}
    allowed = _normalize_list(rule.get("task_allow"))
    return [item for item in allowed if item in STANDARD_TASK_TYPES]
...
def _build_admitted_candidate(
    *,
    ...
    try:
        normalized_priority = max(0, min(99, int(priority if priority is not None else 60)))
    except (TypeError, ValueError):
        normalized_priority = 60

    task_allow = _rule_task_allow(skill_id, routing_rules) or _derive_task_allow(entry)
