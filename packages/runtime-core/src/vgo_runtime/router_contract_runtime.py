...
def _public_admitted_candidates(rows: object, max_rows: int = 12) -> list[dict[str, object]]:
    ...
    for row in list(rows or [])[:max_rows]:
        ...
        public_rows.append({
            ...
            "task_allow": list(row.get("task_allow") or []),
            "skill_md_path": str(row.get("skill_md_path") or ""),
        })
    return public_rows
...
def route_prompt(
    ...
    result = {
        ...
        "custom_admission": {
            ...
            "admitted_candidate_count": len(list(custom_admission.get("admitted_candidates") or [])),
            "admitted_candidates_truncated": len(list(custom_admission.get("admitted_candidates") or [])) > 12,
            "admitted_candidates": _public_admitted_candidates(custom_admission.get("admitted_candidates"), max_rows=12),
        },
    }
