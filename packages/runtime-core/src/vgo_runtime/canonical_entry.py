...
def _resolve_progressive_requested_stage_stop(
    *,
    ...
    if normalized_requested_stage_stop and normalized_requested_stage_stop in progressive_stage_stops:
        return normalized_requested_stage_stop

    return progressive_stage_stops[-1]
...
def _runtime_packet_records_specialist_resolution(runtime_packet: dict[str, Any]) -> bool:
    ...
    return (
        ...
        or (decision_state == "degraded" and resolution_mode == "degraded")
        or (decision_state == "blocked" and resolution_mode == "blocked")
    )
...
def assert_minimum_truth_consistency(
    *,
    ...
    specialist_resolution_recorded = _runtime_packet_records_specialist_resolution(runtime_packet)
    if not selected_skill_ids and not specialist_resolution_recorded:
        raise RuntimeError("canonical truth packet must preserve selected skill or specialist resolution evidence")
    if not selected_skill and not specialist_resolution_recorded:
        raise RuntimeError("canonical truth packet missing route_snapshot selected_skill")
    if selected_skill and specialist_resolution_recorded and selected_skill_ids:
        raise RuntimeError("route_snapshot selected_skill conflicts with specialist resolution evidence")
    specialist_decision = runtime_packet.get("specialist_decision")
    degraded_skill_ids = []
    if isinstance(specialist_decision, dict):
        raw_degraded_skill_ids = specialist_decision.get("degraded_skill_ids") or []
        if isinstance(raw_degraded_skill_ids, list):
            degraded_skill_ids = [str(skill_id).strip() for skill_id in raw_degraded_skill_ids if str(skill_id).strip()]
    if selected_skill_ids and selected_skill not in selected_skill_ids and selected_skill not in degraded_skill_ids:
    ...
    if not divergence_router_skill and not specialist_resolution_recorded:
