...
foreach ($pkg in "runtime-core","contracts","verification-core","installer-core") {
  $env:PYTHONPATH += ";<skill_root>/packages/$pkg/src"
}
...
**Bash (macOS/Linux, including Claude Code):**

```bash
REPO_ROOT='<skill_root>'
WORKSPACE_ROOT="${WORKSPACE_ROOT:-$PWD}"

# Detect Python interpreter (cross-platform: python3 on macOS/Linux, py -3 on Windows)
...
# Build complete PYTHONPATH (all 5 runtime packages required)
PYTHONPATH="$REPO_ROOT/apps/vgo-cli/src"
for pkg in runtime-core contracts verification-core installer-core; do
  PYTHONPATH="$PYTHONPATH:$REPO_ROOT/packages/$pkg/src"
done
export PYTHONPATH
