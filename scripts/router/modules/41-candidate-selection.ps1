...
function Select-PackCandidate {
    ...
    for ($i = 0; $i -lt $filteredCandidates.Count; $i++) {
        ...
        $customKeywordScore = 0.0
        if ($Pack -and $Pack.PSObject.Properties.Name -contains 'custom_admission' -and $Pack.PSObject.Properties.Name -contains 'trigger_keywords') {
            $customKeywordScore = Get-KeywordRatio -PromptLower $PromptLower -Keywords @($Pack.trigger_keywords)
            if ($rule -and ($rule.PSObject.Properties.Name -contains 'requires_positive_keyword_match')) {
                $customKeywordScore = [Math]::Min([double]$customKeywordScore, [double]$positiveScore)
            }
            $keywordScore = [Math]::Max([double]$keywordScore, [double]$customKeywordScore)
        }
        ...
        $scored += [pscustomobject]@{
            ...
            custom_keyword_score = [Math]::Round($customKeywordScore, 4)
