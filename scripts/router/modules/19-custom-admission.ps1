...
function Get-CustomAdmissionTaskAllow {
    param(
        ...
        [AllowEmptyString()] [string]$SkillId = '',
        [AllowNull()] [object]$RoutingRules = $null
    )

    $standard = @('planning', 'coding', 'review', 'debug', 'research')
    if ($RoutingRules -and $RoutingRules.skills -and -not [string]::IsNullOrWhiteSpace($SkillId)) {
        $ruleKeys = @($RoutingRules.skills.PSObject.Properties.Name)
        if ($ruleKeys -contains $SkillId) {
            $rule = $RoutingRules.skills.$SkillId
            if ($rule -and $rule.task_allow) {
                $ruleAllow = @($rule.task_allow | ForEach-Object { Normalize-Key -InputText ([string]$_) } | Where-Object { $_ -in $standard } | Select-Object -Unique)
                if (@($ruleAllow).Count -gt 0) {
                    return @($ruleAllow)
                }
            }
        }
...
function ConvertTo-CustomAdmissionCandidate {
    ...
    $taskAllow = Get-CustomAdmissionTaskAllow -Entry $Entry -SkillId $skillId -RoutingRules $RoutingRules
