...
$customAdmission = Get-CustomAdmissionResult -RepoRoot ([string]$repoRoot) -TargetRoot $resolvedTargetRoot -HostId $HostId -RequestedCanonical $requestedCanonical -RoutingRules $routingRules
...
function ConvertTo-PublicAdmittedCandidates {
    ...
    foreach ($row in @($Rows)) {
        ...
        $index++
        $publicRows += [pscustomobject]@{
            skill_id = [string]$row.skill_id
            manifest_kind = [string]$row.manifest_kind
            pack_id = [string]$row.pack_id
            trigger_mode = [string]$row.trigger_mode
            priority = if ($row.PSObject.Properties.Name -contains 'priority') { [int]$row.priority } else { $null }
            task_allow = if ($row.PSObject.Properties.Name -contains 'task_allow') { @($row.task_allow) } else { @() }
...
$result = [pscustomobject]@{
    ...
    custom_admission = [pscustomobject]@{
        ...
        admitted_candidate_count = @($customAdmission.admitted_candidates).Count
        admitted_candidates_truncated = (@($customAdmission.admitted_candidates).Count -gt 12)
        admitted_candidates = @(ConvertTo-PublicAdmittedCandidates -Rows @($customAdmission.admitted_candidates) -MaxRows 12)
    }
...
if (-not $confirmSkillOptions -and [string]$result.route_mode -eq 'confirm_required' -and [bool]$result.hazard_alert_required -and $result.hazard_alert) {
    $hazardConfirmLines = @(
        [string]$result.hazard_alert.title,
        [string]$result.hazard_alert.message
    )
    if ($result.hazard_alert.reason) {
        $hazardConfirmLines += ("Trigger reason: `{0}`." -f [string]$result.hazard_alert.reason)
    }
    if ($result.hazard_alert.recovery_action) {
        $hazardConfirmLines += [string]$result.hazard_alert.recovery_action
    }
    $hazardConfirmLines += 'Routing confirmation is required before continuing, but no eligible skill option was available from the current route result.'
    $result | Add-Member -NotePropertyName "confirm_ui" -NotePropertyValue ([pscustomobject]@{
        enabled = $true
        pack_id = $null
        selected_skill = $null
        options = @()
        route_decision_contract = $null
        clarification_questions = @()
        rendered_text = ($hazardConfirmLines -join "`n")
        hazard_alert_required = [bool]$result.hazard_alert_required
        truth_level = [string]$result.truth_level
        degradation_state = [string]$result.degradation_state
        hazard_alert = $result.hazard_alert
