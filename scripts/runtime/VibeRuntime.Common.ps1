...
function Resolve-VibeRuntimeMode {
    param(
        ...
        [AllowEmptyString()] [string]$DefaultMode = 'approved_continuous_governed'
    )
    ...
    if ($normalized -notin @('interactive_governed', 'approved_continuous_governed')) {
...
function New-VibeSpecialistDecisionProjection {
    ...
    $unavailableNativeEntrypointSkillIds = @($degradedDispatchArray | Where-Object {
        $nativeUsageRequired = if (Test-VibeObjectHasProperty -InputObject $_ -PropertyName 'native_usage_required') {
            [bool]$_.native_usage_required
        } else {
            $true
        }
        $nativeEntrypoint = if (Test-VibeObjectHasProperty -InputObject $_ -PropertyName 'native_skill_entrypoint') {
            [string]$_.native_skill_entrypoint
        } else {
            ''
        }
        $nativeUsageRequired -and [string]::IsNullOrWhiteSpace($nativeEntrypoint)
    } | ForEach-Object {
        if (Test-VibeObjectHasProperty -InputObject $_ -PropertyName 'skill_id') { [string]$_.skill_id } else { '' }
    } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | Select-Object -Unique)
    $onlyUnavailableNativeEntrypoints = (
        @($degradedSkillIds).Count -gt 0 -and
        @($degradedSkillIds).Count -eq @($unavailableNativeEntrypointSkillIds).Count -and
        @($approvedDispatchSkillIds).Count -eq 0 -and
        @($blockedSkillIds).Count -eq 0 -and
        @($localSuggestionSkillIds).Count -eq 0
    )

    $decisionState = if (@($approvedDispatchSkillIds).Count -gt 0) {
        'approved_dispatch'
    } elseif ($onlyUnavailableNativeEntrypoints) {
    ...
    $resolutionMode = switch ($decisionState) {
        ...
        default {
            if ($onlyUnavailableNativeEntrypoints -or [int]$recommendationCountResolved -eq 0) {
...
function New-VibeRuntimeInputPacketProjection {
    ...
    if ([string]::IsNullOrWhiteSpace($routerSelectedSkill) -and $null -ne $SkillRouting -and (Test-VibeObjectHasProperty -InputObject $SkillRouting -PropertyName 'selected')) {
        $selectedSkillRoutingIds = @($SkillRouting.selected | ForEach-Object {
                if ($null -ne $_ -and (Test-VibeObjectHasProperty -InputObject $_ -PropertyName 'skill_id')) { [string]$_.skill_id }
            } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
        if (@($selectedSkillRoutingIds).Count -gt 0) {
            $routerSelectedSkill = [string]$selectedSkillRoutingIds[0]
        }
    }
