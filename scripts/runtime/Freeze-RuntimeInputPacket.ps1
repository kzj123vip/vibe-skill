...
function Split-VibeSpecialistDispatch {
    ...
    foreach ($recommendation in @($Recommendations)) {
        ...
        $nativeEntrypoint = if ($recommendation.PSObject.Properties.Name -contains 'native_skill_entrypoint') {
            [string]$recommendation.native_skill_entrypoint
        } else {
            ''
        }
        $nativeUsageRequired = if ($recommendation.PSObject.Properties.Name -contains 'native_usage_required') {
            [bool]$recommendation.native_usage_required
        } else {
            $true
        }
        if ($nativeUsageRequired -and [string]::IsNullOrWhiteSpace($nativeEntrypoint)) {
            ...
            $promotionOutcomes += [pscustomobject]@{
                ...
                destructive_reason_codes = @('missing_native_skill_entrypoint')
                contract_complete = $false
                recommended_promotion_action = 'degrade_missing_contract'
            }
            continue
        }
